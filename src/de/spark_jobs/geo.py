...
# --- HELPER: PREPARE DATA FOR BROADCASTING ---

def prepare_station_lookup(station_df: pd.DataFrame) -> dict:
    """
    Converts Station DataFrame into a fast Lookup Dictionary.
    CRITICAL OPTIMIZATION: Pre-builds the BallTree here so workers
    don't have to build it for every single row.
    """
    lookup = {}
    for district, group in station_df.groupby('district'):
        ...
        tree = None
        if len(codes) > 1:
            # Use Haversine metric for lat/lon distance
            tree = BallTree(np.radians(coords), metric='haversine')
            
        lookup[district] = {
            'coords': coords,
            'codes': codes,
            'tree': tree  # <--- Storing the heavy object to reuse later
        }
    return lookup
...
def verify_in_bma_udf(lat: pd.Series, lon: pd.Series) -> pd.Series:
    ...
    # Spatial Join (Fast for small shapefiles like BMA)
    joined = gpd.sjoin(gdf_points, gdf_shape, how="inner", predicate="within")
    
    # Return Boolean Series (aligned with input index)
    return pd.Series(gdf_points.index.isin(joined.index))

# --- 2. NEAREST STATION UDF (Optimized) ---

@pandas_udf(StringType())
def get_nearest_station_udf(district: pd.Series, lat: pd.Series, lon: pd.Series) -> pd.Series:
    """
    Finds nearest station code. 
    Robustly handles 'เขต' prefix mismatch and uses pre-built BallTrees.
    """
    ...
    for d, la, lo in zip(district, lat, lon):
        ...
        # 2. Smart Lookup (The "Fuzzy" Fix)
        # Try exact match first
        target_data = lookup_map.get(d)
        
        # If not found, try removing "เขต" (District prefix)
        if target_data is None and isinstance(d, str) and d.startswith("เขต"):
            target_data = lookup_map.get(d.replace("เขต", ""))
            
        # If still not found, try ADDING "เขต" (just in case station file has it)
        if target_data is None and isinstance(d, str) and not d.startswith("เขต"):
            target_data = lookup_map.get(f"เขต{d}")

        # If STILL not found, give up (This district has no stations)
        if target_data is None:
            results.append(None)
            continue
            
        # 3. FAST Query (Using Pre-built Tree)

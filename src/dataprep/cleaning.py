...
def convert_types(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    # Use coerce to handle bad formats gracefully
...
def parse_coordinates(df: pd.DataFrame) -> pd.DataFrame:
    """Splits '100.5,13.7' into Longitude/Latitude columns."""
    df = df.copy()
...
def parse_type_column(df: pd.DataFrame, explode: bool = False) -> pd.DataFrame:
    """Cleans the 'type' column which contains strings like '{flood,traffic}'"""
